...
let darwinProvider; // libreria a usar para hacer las llamadas AJAX
...
const addProvider = (provider, factory) => {
    darwinProvider = provider;
    return Promise.resolve(factory);
};
const configHeaders = (headers, factory) => {
    return Promise.resolve(new Proxy(factory, {
        apply(target, thisArg, argumentsList) {
            const api = target(); // obtiene la api que hubiera configurada hasta este momento
            return {
                get(...args) {
                    const allHeaders = Object.assign({}, args[1] ? args[1].headers : {}, headers);
                    return api.get(args[0], { headers: allHeaders });
                },
                post(...args) {
                    const allHeaders = Object.assign({}, args[2] ? args[2].headers : {}, headers);
...
const configToken = (tokenServiceUrl, client, factory) => {
        ...
        api.post(tokenServiceUrl, 'grant_type=client_credentials', {
            ...
            resolve(new Proxy(factory, {
                apply(target, thisArg, argumentsList) {
                    ...
                    return {
                        get(...args) {
                            const allHeaders = Object.assign({}, args[1] ? args[1].headers : {}, tokenHeader);
                            return api.get(args[0], { headers: allHeaders });
                        },
                        post(...args) {
                            const allHeaders = Object.assign({}, args[2] ? args[2].headers : {}, tokenHeader);
                            return api.post(args[0], args[1], { headers: allHeaders });
                        },
                    };
                },
...
(async () => {
    const darwinHttpFactoryBuilder = pipeP(addProviderCurrified(axios$1), configHeadersCurrified({ 'X-Requested-With': 'DarwinXMLHttpRequest' }), configTokenCurrified('https://accounts.spotify.com/api/token', {
